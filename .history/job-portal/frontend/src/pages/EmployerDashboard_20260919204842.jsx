...
const handleSendInterviewCall = async (e) => {
  ...
  if (!interviewPhone || interviewPhone.trim() === '') {
    alert('Please enter candidate phone number!');
    return;
  }
  ...
  try {
    const payload = {
      ...
      applicantName: selectedApplicant?.applicant?.name || 'Candidate',
      jobTitle: selectedApplicant?.job?.title || 'Job Position',
                        ...
                        {job.applications.map((app) => (
                          <tr key={app.id} className="hover:bg-purple-50/30 transition">
                            ...
                            <td className="py-4 px-6 text-right">
                              {/* Application Status Badge and Action Buttons */}
<div className="flex items-center gap-2">
  {/* ১. স্ট্যাটাস যদি SHORTLISTED হয় তবেই Send Interview SMS বাটন দেখাবে */}
  {app.status === 'SHORTLISTED' && (
    <button
      onClick={() => handleOpenInterviewModal(app)}
      className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium text-xs rounded-lg shadow transition flex items-center gap-1.5 cursor-pointer"
    >
  ...
  <button
    ...
    className={`px-3 py-1.5 text-xs font-medium rounded-lg transition ${
      app.status === 'REJECTED'
        ? 'bg-rose-100 text-rose-600 cursor-not-allowed opacity-60'
