...
export default function Navbar() {
  ...
  // Helper function: শুধুমাত্র বর্তমান পাথ ম্যাচ করলেই Active Class দেবে
  const getNavLinkClass = (path) => {
    const isActive = location.pathname === path;
    return `px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-200 ${
      isActive
        ? 'text-white bg-white/20 shadow-sm border border-white/20 font-bold'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="relative flex items-center justify-between h-16 sm:h-18">
          ...
          <div className="hidden md:flex items-center gap-1.5 absolute left-1/2 -translate-x-1/2">
            <Link 
              to="/" 
              className={location.pathname === '/' || location.pathname === '/home' ? 'px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-bold text-white bg-white/20 shadow-sm border border-white/20 transition-all' : 'px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-semibold text-purple-200/80 hover:text-white hover:bg-white/10 transition-all'}
            >
              Home
            </Link>
            ...
            {user && (
              <Link
                ...
                className={`px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-semibold transition-all flex items-center gap-1.5 ${
                  ['/dashboard', '/employerdashboard', '/applicantdashboard'].includes(location.pathname)
                    ? 'text-white bg-white/20 shadow-sm border border-white/20 font-bold'
                    : 'text-purple-200/80 hover:text-white hover:bg-white/10'
                }`}
              >
            ...
            {isAdmin && (
              <Link
                ...
                className={`px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-bold transition-all flex items-center gap-1.5 ${
                  ['/admin', '/admindashboard'].includes(location.pathname)
                    ? 'text-amber-300 bg-amber-500/30 border border-amber-400/50 shadow-sm'
          ...
          <div className="hidden md:flex items-center gap-3 z-10">
            ...
            {user ? (
              ...
              <div className="flex items-center gap-2.5">
                <Link
                  to="/login"
                  className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white font-semibold text-xs sm:text-sm rounded-xl border border-white/20 transition shadow-sm"
                >
                  Login
                </Link>
                <Link
                  to="/register"
                  className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs sm:text-sm rounded-xl transition shadow-md"
                >
                  Register
                </Link>
              </div>
          ...
          <div className="space-y-1 text-sm font-semibold">
            <Link
              ...
              className={`block px-3 py-2 rounded-xl transition ${
                location.pathname === '/' || location.pathname === '/home' ? 'bg-white/20 text-white font-bold' : 'hover:bg-white/10'
              }`}
            >
              Home
            </Link>
            <Link
              ...
              onClick={() => setIsMobileMenuOpen(false)}
              className={`block px-3 py-2 rounded-xl transition ${
                location.pathname === '/jobs' ? 'bg-white/20 text-white font-bold' : 'hover:bg-white/10'
              }`}
            >
              Jobs
            </Link>
            <Link
              ...
              onClick={() => setIsMobileMenuOpen(false)}
              className={`block px-3 py-2 rounded-xl transition ${
                location.pathname === '/companies' ? 'bg-white/20 text-white font-bold' : 'hover:bg-white/10'
              }`}
            >
              Companies
            </Link>
            <Link
              ...
              onClick={() => setIsMobileMenuOpen(false)}
              className={`block px-3 py-2 rounded-xl transition ${
                location.pathname === '/about' ? 'bg-white/20 text-white font-bold' : 'hover:bg-white/10'
              }`}
            >
              About
            </Link>
            <Link
              ...
              onClick={() => setIsMobileMenuOpen(false)}
              className={`block px-3 py-2 rounded-xl transition ${
                location.pathname === '/contact' ? 'bg-white/20 text-white font-bold' : 'hover:bg-white/10'
              }`}
            >
              Contact
            </Link>
