...
export default function EmployerDashboard() {
  ...
  const handleStatusUpdate = async (applicationId, status) => {
    ...
    try {
      ...
      fetchEmployerJobs(); // ডাটা রিফ্রেশ
    } catch (err) {
                    ...
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-gray-200 text-sm">
                        {job.applications.map((app) => (
                          <tr key={app.id}>
                            <td className="p-4 font-semibold text-gray-800">{app.applicant?.name || 'N/A'}</td>
                            <td className="p-4 text-gray-600">{app.applicant?.email || 'N/A'}</td>
                            ...
                            <td className="p-4">
                              {app.resumeUrl ? (
                                <a
                                  ...
                                  className="inline-flex items-center gap-1 text-purple-600 hover:text-purple-800 font-medium underline"
                                >
                                  📄 View Resume
                                </a>
                              ) : (
