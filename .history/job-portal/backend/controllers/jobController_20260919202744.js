...
// Schedule Interview & Send SMS Controller
export const scheduleInterview = async (req, res) => {
  try {
    const { phone, message, date, time, locationOrLink, jobTitle, applicantName } = req.body;

    if (!phone) {
      return res.status(400).json({ message: 'Applicant phone number is missing' });
    }

    // Dynamic SMS message structure
    const smsMessage = message || `Hello ${applicantName || 'Candidate'}, Your interview for '${jobTitle || 'Job'}' is set on ${date} at ${time}. Venue/Link: ${locationOrLink}`;

    // TODO: Apnar SMS Gateway Call (Greenweb, Bulksmsbd, Twilio, etc.)
    console.log(`Sending SMS to ${phone}: ${smsMessage}`);

    // Example SMS Sending Logic (Uncomment & configure with your provider)
    /*
    ...
    */
    ...
    return res.status(200).json({ 
      success: true, 
      message: 'Interview schedule SMS sent successfully!' 
    });
  } catch (error) {
    console.error('Schedule Interview Error:', error);
    return res.status(500).json({ 
      message: error.message || 'Failed to send SMS via Gateway' 
    });
  }
};