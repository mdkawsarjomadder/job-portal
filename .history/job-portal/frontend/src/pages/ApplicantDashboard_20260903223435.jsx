...
export default function ApplicantDashboard() {
  ...
  useEffect(() => {
    // ১. লোকাল স্টোরেজ থেকে লগইন করা ইউজারের ডাটা পড়া
    const storedUser = localStorage.getItem('user');
    if (storedUser) {
      setUser(JSON.parse(storedUser));
    }

    // ২. ব্যাকএন্ড থেকে সব জবের তালিকা লোড করা
    fetchJobs();
  }, []);
 ...
 const handleApply = async (job) => {
  ...
  try {
    const res = await axios.post(
      ...
      {
        jobId: job.id,
        resumeUrl: 'https://example.com/my-resume.pdf', // সাময়িকভাবে ডামি রেজুমি লিংক
      },
      {
        headers: {
