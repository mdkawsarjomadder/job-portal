          ...
          {applications.length === 0 ? (
            ...
            <div className="space-y-4">
              {applications.map((app) => (
                <div
                  key={app.id}
                  className="p-6 border border-slate-200/80 rounded-2xl bg-white shadow-xs hover:border-slate-300 hover:shadow-md transition-all duration-200"
                >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-4 mb-4">
                    ...
                    <span
                      className={`px-3.5 py-1.5 rounded-full text-xs font-semibold border self-start sm:self-center uppercase tracking-wide ${getStatusBadge(
                        app.status
                      )}`}
                    >
                      ● {app.status || 'PENDING'}
                  ...
                  {/* Information Grid */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-6 text-sm text-slate-600 mb-5">
                    {app.job?.location && (
                      <div className="flex items-center gap-2">
                        <span className="text-slate-400">📍 Location:</span>
                        <span className="font-medium text-slate-700">{app.job.location}</span>
                      </div>
                    )}
                    {app.job?.jobType && (
                      <div className="flex items-center gap-2">
                        <span className="text-slate-400">💼 Schedule:</span>
                        <span className="font-medium text-slate-700">{app.job.jobType}</span>
                      </div>
                    )}
                    {app.job?.salary && (
                      <div className="flex items-center gap-2">
                        <span className="text-slate-400">💰 Salary:</span>
                        <span className="font-medium text-slate-700">${Number(app.job.salary).toLocaleString()}</span>
                      </div>
                    )}
                    <div className="flex items-center gap-2">
                      <span className="text-slate-400">📅 Applied On:</span>
                      <span className="font-medium text-slate-700">{new Date(app.appliedAt).toLocaleDateString()}</span>
                    </div>
                  </div>
                  ...
                  {app.resumeUrl && (
                    <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                      <span className="text-xs text-slate-400 font-medium">Submitted Documents</span>
                      <a
                        href={app.resumeUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-slate-100 hover:bg-purple-50 text-slate-700 hover:text-purple-700 border border-slate-200 hover:border-purple-200 text-xs font-semibold rounded-lg transition"
                      >
                        📄 View Resume
                      </a>
                    </div>
