...
export default function EmployerDashboard() {
  ...
  const [statusModalData, setStatusModalData] = useState(null); // { applicationId, status, applicantName }
...
// Modal খুলার সময় phone সেট করার লজিক
const handleOpenInterviewModal = (applicantData) => {
  setSelectedApplicant(applicantData);
  // যদি প্রার্থীর ডাটাবেজে phone না থাকে, তবে খালি string দিয়ে ইনপুট শুরু হবে
  setInterviewPhone(
    applicantData?.phone || 
    applicantData?.applicant?.phone || 
    ''
  );
  setIsInterviewModalOpen(true);
};

  // Submit Interview & Send SMS
 const handleSendInterviewCall = async (e) => {
  e.preventDefault();
  setIsSendingSms(true);
  const token = localStorage.getItem('token');

  // Candidate Data Extraction with Fallbacks
  const applicantObj = selectedApplicant?.applicant || selectedApplicant?.user || selectedApplicant || {};
  
  // Ensure valid phone number exists
  ...
  const recipientName = applicantObj?.name || selectedApplicant?.applicantName || 'Candidate';
  const jobTitle = selectedApplicant?.jobTitle || 'Job Position';

  if (!recipientPhone) {
    alert('Candidate phone number is missing! Cannot send SMS.');
    setIsSendingSms(false);
    return;
  }

  const smsMessage = `Hello ${recipientName}, Your interview for '${jobTitle}' is scheduled on ${interviewDetails.date} at ${interviewDetails.time}. Mode: ${interviewDetails.type}. Location/Link: ${interviewDetails.locationOrLink}.`;

  try {
    const payload = {
      applicationId: selectedApplicant?.id || selectedApplicant?._id,
      phone: recipientPhone,
      phoneNumber: recipientPhone, // Safety backup key for backend
    ...
    alert(res.data?.message || 'Interview SMS sent successfully!');
    setIsInterviewModalOpen(false);
    setInterviewDetails({ date: '', time: '', type: 'In-Person', locationOrLink: '', note: '' });
    fetchEmployerJobs();
  } catch (err) {
    console.error('Interview SMS Error:', err.response?.data);
    alert(err.response?.data?.message || 'Failed to send Interview SMS');
  } finally {
    setIsSendingSms(false);
  }
};
                            ...
                            <td className="py-4 px-6 text-right">
                              <div className="flex items-center justify-end gap-2">
                                {app.status === 'SHORTLISTED' && (
                                  ...
                                  >
                                    ✉️ Send Interview SMS
                                  </button>
                                )}

                                <button
                                  type="button"
                                  onClick={() => handleOpenStatusModal(app.id, 'SHORTLISTED', app.applicant?.name)}
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleOpenStatusModal(app.id, 'REJECTED', app.applicant?.name)}
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4 border border-slate-100">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <h3 className="text-lg font-bold text-slate-800">
                Call Interview for <span className="text-purple-600">{selectedApplicant?.applicant?.name}</span>
              </h3>
              <button
                type="button"
            ...
            <form onSubmit={handleSendInterviewCall} className="space-y-4 text-sm">
              <div>
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">
                  {interviewDetails.type === 'In-Person' ? 'Office Location Address' : 'Meeting Link (URL)'}
                </label>
                <input
                  type="text"
                  required
                  placeholder={interviewDetails.type === 'In-Person' ? 'e.g. House 12, Banani, Dhaka' : 'https://meet.google.com/xyz'}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-medium rounded-xl transition shadow-sm"
                >
                  {isSendingSms ? 'Sending SMS...' : 'Send Call SMS'}
                </button>
              </div>
            </form>
          </div>
