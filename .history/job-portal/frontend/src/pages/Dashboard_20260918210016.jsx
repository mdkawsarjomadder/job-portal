  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans p-4 sm:p-8 text-left">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* Vertical Profile Dropdown Button */}
<div className="relative" ref={dropdownRef}>
  {/* Main Profile Button (Border and Shadow removed) */}
  <button
    onClick={() => setIsDropdownOpen(!isDropdownOpen)}
    className="flex flex-col items-center gap-1.5 p-2 px-3 bg-gray-50 hover:bg-gray-100 rounded-2xl transition focus:outline-none"
  >
  ...
  {/* Dropdown Menu (Border & Shadow removed) */}
  {isDropdownOpen && (
    <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl py-2 z-50">
      <div className="px-4 py-3 flex items-center gap-3">
        {/* Dropdown Header Avatar */}
        <div className="w-10 h-10 rounded-full bg-purple-100 text-purple-700 font-extrabold text-sm flex items-center justify-center shrink-0 overflow-hidden">
          {user?.avatar ? (
            <img src={user.avatar} alt={user.name} className="w-full h-full object-cover" />
          ) : (
            user?.name?.charAt(0).toUpperCase() || 'U'
