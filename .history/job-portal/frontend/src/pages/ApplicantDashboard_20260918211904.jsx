...
export default function ApplicantDashboard() {
  ...
  // API থেকে লেটেস্ট Avatar সহ ইউজার ডাটা ফেচ করা
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              {/* Profile Avatar */}
              <div className="w-10 h-12 rounded-full bg-purple-600 text-white font-bold text-base flex items-center justify-center overflow-hidden border-2 border-purple-200 shadow-sm shrink-0">
                {user?.avatar ? (
                  <img 
                    src={user.avatar} 
                    alt={user.name} 
              ...
              {/* User Name */}
              <span className="text-xs font-semibold text-gray-700 mt-1 max-w-[120px] truncate text-center">
                {user?.name || 'Profile'}
              </span>
            </button>

            {/* Dropdown Menu - Position Fixed to Align Correctly */}
            {isDropdownOpen && (
              <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 p-3 z-50 transform origin-top-right transition-all">
                {/* Header User Details */}
                ...
                <div className="space-y-1">
                  <button
                    ...
                    className="w-full text-left px-3 py-2.5 text-xs font-semibold text-gray-700 hover:bg-purple-50 hover:text-purple-700 rounded-xl transition flex items-center gap-2"
                  >
                    ✏️ Edit Profile
                  </button>
                  ...
                  <button
                    ...
                    className="w-full text-left px-3 py-2.5 text-xs font-semibold text-red-600 hover:bg-red-50 rounded-xl transition flex items-center gap-2"
                  >
                    🚪 Logout
                  </button>
                </div>
