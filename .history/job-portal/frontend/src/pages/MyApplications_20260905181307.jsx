          ...
          {applications.length === 0 ? (
            ...
            <div className="space-y-4">
              {applications.map((app) => (
                <div
                  key={app.id}
                  className="p-6 border border-slate-200 rounded-xl bg-slate-50/30 flex flex-col md:flex-row md:items-start justify-between gap-4 hover:border-slate-300 transition"
                >
                  <div className="space-y-2">
                    ...
                    {app.job?.employer && (
                      <div className="text-sm text-slate-600 space-y-0.5">
                        <p><span className="font-semibold text-slate-700">Name:</span> {app.job.employer.name}</p>
                        <p><span className="font-semibold text-slate-700">Email:</span> ({app.job.employer.email})</p>
                      </div>
                    )}
                    ...
                    <div className="text-sm text-slate-600 space-y-0.5 pt-1">
                      {app.job?.location && (
                        <p><span className="font-semibold text-slate-700">Location:</span> {app.job.location}</p>
                      )}
                      {app.job?.jobType && (
                        <p><span className="font-semibold text-slate-700">Job Schedule:</span> {app.job.jobType}</p>
                      )}
                      {app.job?.salary && (
                        <p><span className="font-semibold text-slate-700">Salary:</span> ${Number(app.job.salary).toLocaleString()}</p>
                      )}
                      <p><span className="font-semibold text-slate-700">Date:</span> {new Date(app.appliedAt).toLocaleDateString()}</p>
                    </div>
                    ...
                    {app.resumeUrl && (
                      <div className="pt-2">
                        ...
                        <a
                          ...
                          className="inline-flex items-center gap-1 text-sm font-semibold text-purple-600 hover:text-purple-800 hover:underline transition"
                        >
                  ...
                  <div className="shrink-0">
                    <span
                      className={`px-3.5 py-1.5 rounded-full text-xs font-semibold border inline-block ${getStatusBadge(
                        app.status
                      )}`}
                    >
                      {app.status || 'PENDING'}
